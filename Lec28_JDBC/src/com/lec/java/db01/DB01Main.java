...
//			sql_insert ="INSERT INTO test_member VALUES("+no+",'"+name+"',SYSDATE)";
			...
			int cnt = stmt.executeUpdate(sql_insert); //'DML'명령은 executeUpdate() 로 실행
											//리턴값은 int
			System.out.println(cnt + "개 row가 insert 됨");
			
			System.out.println();
			String sql_select = "SELECT mb_no,mb_name,mb_birthdate FROM test_member";
			System.out.println(sql_select);
			
			rs = stmt.executeQuery(sql_select); //'SELECT' 밒 기타 쿼리의 경우 EXECUTEqUERY() 로 실행
											//리턴값은  ResultSet 객체
			
			// 7.ResultSet에 담겨 있는 record 확인
			// 7-1 컬럼 이름으로 받기
	//		System.out.println();
//			while(rs.next()) { //next() : record 하나 추출하고 true return, 더이상 record 없으면 false return
...
//			}
			...
			// 7-3 null처리 하기
			//getxxx()로 가져온 데이터가 null 값이면 null 리턴.
//			System.out.println();
//			while(rs.next()) { //next() : record 하나 추출하고 true return, 더이상 record 없으면 false return
//				String no = rs.getString("mb_no"); //getxxx()에 '컬럼명' 혹은 '별명' 명시
//				if(no == null) no="";
