...
public class RegExp02Main {
	...
	public static void main(String[] args) {
		...
		System.out.println();
		regex = "(My)(....)";  // 정규표현식에 () 사용, 그룹 2개 짜리 정규표현식
		...
		while(matcher.find()) {
			...
			System.out.println("\t group(0): "+matcher.group(0)+"{"+ matcher.start(0)+"~"+matcher.end(0)+"}"); //0번은 전체
			System.out.println("\t group(1): "+matcher.group(1)+"{"+ matcher.start(1)+"~"+matcher.end(1)+"}"); //1번은 첫번째 그룹 
			System.out.println("\t group(2): "+matcher.group(2)+"{"+ matcher.start(2)+"~"+matcher.end(2)+"}"); //2번은 두번째 그룹
	...
	public static void regExpTest(String regex, String input) {
		...
		if(matchCount == 0) System.out.println("매칭 없음");
		System.out.println();
