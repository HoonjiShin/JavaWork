 ...
 * 3.프로젝트 우클릭 - Configure Build Path - Library tab - Add Jars(혹은 라이브러리 추가) - 추가 할 라이브러리 선택
 * 
 * 
 * 네이버 뉴스 캐스트
 */
public class Crawl01Main {
	...
	public static void main(String[] args) throws IOException {
		...
		//Document 나 Element 객체의 select(), selectFirst() method로 특정 element들을 추출
		System.out.println("[네이버 뉴스 캐스트]");
		
		element = doc.selectFirst("#news_cast"); //검색된 element들 중 최초 1개만 element 로 리턴
		//System.out.println(element.outerHtml());
		
		Elements newsElement = doc.select("#news_cast li.ca_item"); //검색된 element 들이 담겨 있는 elements 리턴
		
		//목록 크롤링 할시, 내가 원하는 개수만큼 크롤링 되었는지 우선 확인.
		System.out.println(newsElement.size()+"개");
